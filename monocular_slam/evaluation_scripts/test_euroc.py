...
def image_stream(datapath, image_size=[320, 512], stereo=False, stride=1):
    ...
    d_r = np.array([-0.28368365, 0.07451284, -0.00010473, -3.555907e-05, 0.0]).reshape(
        5
    )
    R_r = np.array(
        [
    ...
    for t, (imgL, imgR) in enumerate(zip(images_left, images_right)):
        ...
        images = [
            cv2.remap(
                cv2.imread(imgL), map_l[0], map_l[1], interpolation=cv2.INTER_LINEAR
            )
        ]
        if stereo:
            images += [
                cv2.remap(
                    cv2.imread(imgR), map_r[0], map_r[1], interpolation=cv2.INTER_LINEAR
                )
            ]
...
if __name__ == "__main__":
    ...
    for t, image, intrinsics in tqdm(
        image_stream(args.datapath, stereo=args.stereo, stride=2)
    ):
