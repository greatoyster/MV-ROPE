...
from droid_net import upsample_flow
...
class FactorGraph:
    ...
    def rm_keyframe(self, ix):
        ...
        with self.video.get_lock():
            ...
            self.video.obj_masks[ix] = self.video.obj_masks[ix + 1]
            self.video.nocs[ix] = self.video.nocs[ix + 1]
            self.video.metas[ix] = self.video.metas[ix + 1]
    ...
    def update(
        ...
        # TODO: export optical flow
        # if obj_enabled:
        #         ht, wd = self.coords0.shape[0:2]
        #         print("shape of coord0", self.coords0.shape)
        #         flow = coords1- self.coords0
        #         ts = torch.index_select(self.video.tstamp,0, torch.unique(self.ii))
        #         for idx, (i,j) in enumerate(zip(self.ii,self.jj)):
        #             i = self.video.tstamp[i]
        #             j = self.video.tstamp[j]
        #             if i == 101:
        #                 i_index = torch.where(ts==i)
        #                 if torch.sum(ts == 101)!=1:
        #                     continue
        #                 a=flow[0,idx,:,:,:]
        #                 b=upmask[:,i_index,:,:,:]
        #                 a=a.view(-1, ht, wd, 2).permute(0,3,1,2).contiguous()
        #                 b=b.contiguous()
        #                 upsampled_flow = upsample_flow(a,b)
        #                 print(f"saving optical flow: frame {i} to frame {j}")
        #                 fname = f"flow_{int(i)}_{int(j)}.pth"
        #                 torch.save(upsampled_flow, fname)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.upsample and obj_enabled:
                self.video.upsample(torch.unique(self.ii), upmask)
                self.video.object_centric_ba(self.ii)
        self.age += 1
