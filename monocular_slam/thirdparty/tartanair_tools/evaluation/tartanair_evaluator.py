...
class TartanAirEvaluator:
    ...
    def evaluate_one_trajectory(self, gt_traj, est_traj, scale=False, title=""):
        ...
        plot_traj(
            np.matrix(gt_ate_aligned),
            np.matrix(est_ate_aligned),
            vis=False,
            savefigname="figures/%s.pdf" % title,
            title=title,
        )
...
if __name__ == "__main__":
    ...
    result = aicrowd_evaluator.evaluate_one_trajectory(
        "pose_gt.txt", "pose_est.txt", scale=True
    )
    print(result)
