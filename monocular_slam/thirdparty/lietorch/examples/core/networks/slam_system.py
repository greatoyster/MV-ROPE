...
class SLAMSystem(RaftSLAM):
    ...
    def initialize(self, ht, wd):
        ...
        self.fmaps = torch.zeros(
            1, self.mem, 128, ht, wd, device="cuda", dtype=torch.half
        )
    ...
    def transform_project(self, ii, jj, **kwargs):
        ...
        return pops.projective_transform(
            self.poses, self.disps, self.intrinsics, ii, jj, **kwargs
        )
    ...
    def moba(self, num_steps=5, is_init=False):
        ...
        with autocast(enabled=True):
            ...
            for i in range(num_steps):
                ...
                self.factors.hidden, delta, weight = self.update(
                    self.factors.hidden, self.factors.inputs, corr, flow
                )
        ...
        if self.frontend is not None:
            ...
            for i, ix in enumerate(keyframe_graph.ixs):
                self.add_point_cloud(
                    ix, images[:, i], Gs[:, i], depths[:, i], intrinsics[:, i], s=4
                )
        ...
        with autocast(True):
            ...
            for step in range(num_steps):
                ...
                for i in range(0, ii.shape[0], s):
                    ...
                    flow1 = (
                        residual[:, i : i + s].permute(0, 1, 4, 2, 3).clamp(-32.0, 32.0)
                    )

                    corr1 = torch.cat([corr1, mask[:, ii1, None]], dim=2)
                    flow1 = torch.cat([flow1, mask[:, ii1, None]], dim=2)

                    net1, delta, weight = self.update(
                        net[:, i : i + s], inp[:, ii1], corr1, flow1
                    )
                    net[:, i : i + s] = net1

                    targets_list += [coords[:, i : i + s] + delta.float()]
                    weights_list += [
                        weight.float() * torch.as_tensor([1.0, 1.0, 0.0]).cuda()
                    ]
