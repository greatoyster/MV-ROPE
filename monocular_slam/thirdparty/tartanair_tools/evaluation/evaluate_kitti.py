...
def calculate_sequence_error(
    poses_gt, poses_result, lengths=[10, 20, 30, 40, 50, 60, 70, 80]
):
    # error_vetor
    errors = []
    ...
    for first_frame in range(0, len(poses_gt), step_size):
        ...
        for i in range(0, num_lengths):
            ...
            pose_delta_gt = np.linalg.inv(poses_gt[first_frame]).dot(
                poses_gt[last_frame]
            )
            pose_delta_result = np.linalg.inv(poses_result[first_frame]).dot(
